...
import { DefaultErrorCode, getErrorDefault } from '../response.constants';
import {
  getErrorsFromUnknown,
  getMessageFromUnknown,
  isBaseErrorLike,
  isResponseBodyRecord,
} from '../common/error.guards';
import { NormalizeExceptionOptions, NormalizedException } from '../types';

const getFallbackError = <TCode extends string>(
  statusCode: number,
  options: NormalizeExceptionOptions<ErrorDetails | undefined, TCode>,
): ErrorType<TCode> => {
...
>(
  ...
  options: NormalizeExceptionOptions<TErrors, TCode>,
...
>(
  ...
  const customNormalized = options.customErrorFactory?.(exception);
